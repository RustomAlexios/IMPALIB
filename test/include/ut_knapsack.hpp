...
void ut_forward_backward(string& ut_name) {
    ...
    for (int department_index = 0; department_index < N_DEPARTMENTS; department_index++) {
        copy(teams_weights_per_department_pure + N_TEAMS * department_index, teams_weights_per_department_pure + N_TEAMS * (department_index + 1),
             teams_weights_per_department[department_index].begin());
    }
    ...
    for (int department_index = 0; department_index < N_DEPARTMENTS; department_index++) {
        ...
        copy(non_zero_weight_indices_arr_pure + max_size_non_zero_weight * department_index,
             non_zero_weight_indices_arr_pure + non_zero_weight_indices_sizes_pure[department_index] + max_size_non_zero_weight * department_index, non_zero_weight_indices[department_index].begin());
    }
    ...
    for (int department_index = 0; department_index < N_DEPARTMENTS; department_index++) {
        ...
        if (ut_name == "KnapsackForward") {
            ...
            modelKnapsacks.forward(department_index, stage_forward_messages, max_state_department, non_zero_weight_indices, non_zero_weight_indices_sizes_pure, teams_weights_per_department,
                                   team_to_knapsack_m);
            ...
            modelKnapsacks.backward(department_index, stage_backward_messages, max_state_department, non_zero_weight_indices, non_zero_weight_indices_sizes_pure, teams_weights_per_department,
                                    team_to_knapsack_m);
...
void ut_extrinsic_output_department(string& ut_name) {
    ...
    for (int department_index = 0; department_index < N_DEPARTMENTS; department_index++) {
        ...
        modelKnapsacks.extrinsic_output_department_lhs(teams_weights_per_department, stage_forward_messages, team_to_knapsack_m, department_index, stage_backward_messages, max_state_department,
                                                       extrinsicOutputDepartment);
    }
...
void ut_team_to_knapsack_update(string& ut_name) {
    ...
    Knapsack modelKnapsacks(N_DEPARTMENTS, N_TEAMS, FILT_FLAG, ALPHA);
    modelKnapsacks.team_to_knapsack_update(non_zero_weight_indices, team_to_knapsack_m, reward_team, extrinsic_output_department, oric_to_team_m);
...
void ut_process_extrinsic_output_department(string& ut_name) {
    ...
    for (int department_index = 0; department_index < N_DEPARTMENTS; department_index++) {
        copy(extrinsic_output_department_pure + N_TEAMS * department_index, extrinsic_output_department_pure + N_TEAMS * (department_index + 1),
             extrinsic_output_department_dummy[department_index].begin());
    }
    ...
    for (int iter = 0; iter < N_ITER; iter++) {
        for (int department_index = 0; department_index < N_DEPARTMENTS; department_index++) {
            modelKnapsacks.process_extrinsic_output_department(department_index, iter, extrinsic_output_department_dummy, extrinsic_output_department);
        }
    }
