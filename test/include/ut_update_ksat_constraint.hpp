...
void ut_ksat_constraint(string& ut_name){
    ...
    for (int constraint_index=0; constraint_index<NUM_CONSTRAINTS; constraint_index++){
    copy (constraints_connections_type_pure + K_VARIABLE*constraint_index, constraints_connections_type_pure+K_VARIABLE*(constraint_index+1), constraints_connections_type[constraint_index].begin() );
    }
    ...
    for (int constraint_index=0; constraint_index<NUM_CONSTRAINTS; constraint_index++){
    copy (variable_ec_to_ksat_constraint_m_pure + NUM_VARIABLES*constraint_index, variable_ec_to_ksat_constraint_m_pure+NUM_VARIABLES*(constraint_index+1), variable_ec_to_ksat_constraint_m[constraint_index].begin() );
    }
