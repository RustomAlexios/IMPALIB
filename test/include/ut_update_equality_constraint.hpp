...
void ut_eq_constraint_kc_mwm(string& ut_name) {
    ...
    if (ut_name == "TeamEc2OricUpdate") {
        ...
        for (int department_index = 0; department_index < N_DEPARTMENTS; department_index++) {
            copy(extrinsic_output_department_pure + N_TEAMS * department_index, extrinsic_output_department_pure + N_TEAMS * (department_index + 1),
                 extrinsic_output_department[department_index].begin());
        }
...
void ut_equality_constraint_tsp(string& ut_name) {
    ...
    for (int edge_variable_index = 0; edge_variable_index < N_EDGE_VARIABLES; edge_variable_index++) {
        copy(edge_degree_constraint_cost_pure + N_NODES * edge_variable_index, edge_degree_constraint_cost_pure + N_NODES * (edge_variable_index + 1),
             edge_degree_constraint_cost[edge_variable_index].begin());
    }
    ...
    for (int edge_variable_index = 0; edge_variable_index < N_EDGE_VARIABLES; edge_variable_index++) {
        copy(degree_constraint_to_eq_constraint_m_pure + N_NODES * edge_variable_index, degree_constraint_to_eq_constraint_m_pure + N_NODES * (edge_variable_index + 1),
             degree_constraint_to_eq_constraint_m[edge_variable_index].begin());
    }
    ...
    if (ut_name == "EdgeEc2DegreeConstraintRelaxedGraphUpdate") {
        ...
        for (int subtour_index = 0; subtour_index < N_SUBTOURS; subtour_index++) {
            copy(subtour_constraints_to_edge_ec_m_pure + N_EDGE_VARIABLES * subtour_index, subtour_constraints_to_edge_ec_m_pure + N_EDGE_VARIABLES * (subtour_index + 1),
                 subtour_constraints_to_edge_ec_m[subtour_index].begin());
        }

        vector<vector<impalib_type>> edge_ec_to_subtour_constraints_m(N_SUBTOURS, vector<impalib_type>(N_EDGE_VARIABLES, zero_value));

        edge_ec_to_subtour_constraints_m = modelEqualityConstraint.edge_ec_to_subtour_constraints_update(delta_S_indices_list, cost_edge_variable, degree_constraint_to_eq_constraint_m,
                                                                                                         subtour_constraints_to_edge_ec_m, edge_connections);
        ...
        modelEqualityConstraint.edge_ec_to_degree_constraint_augmented_graph_update(degree_constraint_to_eq_constraint_m, subtour_constraints_to_edge_ec_m, edge_connections,
                                                                                    edge_degree_constraint_cost, edge_ec_to_degree_constraint_m);
...
void ut_equality_constraint_ksat(string& ut_name) {
    ...
    for (int variable_index = 0; variable_index < NUM_VARIABLES; variable_index++) {
        if (find(used_variables.begin(), used_variables.end(), variable_index) != used_variables.end()) {
            ...
            copy(variables_connections_pure + connections_size_old, variables_connections_pure + connections_size_old + connections_size, variables_connections[variable_index].begin());
    ...
    for (int constraint_index = 0; constraint_index < NUM_CONSTRAINTS; constraint_index++) {
        copy(ksat_constraint_to_eq_constraint_m_pure + NUM_VARIABLES * constraint_index, ksat_constraint_to_eq_constraint_m_pure + NUM_VARIABLES * (constraint_index + 1),
             ksat_constraint_to_eq_constraint_m[constraint_index].begin());
    }
    ...
    if (ut_name == "VariableEc2KsatConstraintUpdate") {
        ...
        modelEqualityConstraint.variable_ec_to_ksat_constraint_update(ksat_constraint_to_eq_constraint_m, variable_ec_to_ksat_constraint_m, used_variables, incoming_metrics_cost,
