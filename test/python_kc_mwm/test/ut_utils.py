...
def check_forward_backward_results(sub_test_num, total_sub_tests, ut_name, rtol=1e-05, atol=1e-08):
    ...
    for number in file_numbers:
        flag_failed = True
        if ut_name == "KnapsackForward":
            f_path_pure = "../ut_results/forward_pure" + str(number)
            f_path_wrapper = "../ut_results/forward_wrapper" + str(number)
        elif ut_name == "KnapsackBackward":
            f_path_pure = "../ut_results/backward_pure" + str(number)
            f_path_wrapper = "../ut_results/backward_wrapper" + str(number)
...
def team_reward_generation(available_combinations, N_u, team_types):
    ...
    for department_index in range(0, len(N_u)):
