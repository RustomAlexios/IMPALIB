...
class GraphicalModelKcMwm:
    ...
    def initialize(self, input_load, test_flag=False):
        ...
        self.project_ineq_constraint = InequalityConstraint(
            self.num_departments,
            self.num_teams,
            self.num_projects,
            self.unbalanced_flag,
        )
        self.model_oric = OrInequalityConstraint(
            self.num_departments,
            self.num_teams,
            self.num_projects,
            self.unbalanced_flag,
        ...
        self.model_eq_constraint = EqualityConstraintKcMwm(
            self.num_departments,
            self.num_teams,
            self.num_projects,
            self.reward_team,
    ...
    def iterate(self):
        for iter in range(0, self.num_iterations):
            for bin_index in range(0, self.num_departments):
                self.model_knapsacks.forward_backward(
                    self.max_state[bin_index],
                    self.team_to_knapsack_m[bin_index],
                    np.array(self.teams_weights_per_department[bin_index]),
                )

                self.model_knapsacks.extrinsic_output_department_lhs(
                    bin_index,
                    self.max_state[bin_index],
                    self.team_to_knapsack_m[bin_index],
                    np.array(self.teams_weights_per_department[bin_index]),
    ...
    def pre_processing_analysis(self):
        ...
        for i in range(0, len(selected_teams_indices)):
            ...
            print(
                ...
                " & is in units ",
                np.array(available_combinations[location_team]) - 1,
            )
    ...
    def results_analysis(self):
        ...
        for i in range(0, len(intermediate_selected_teams_indices)):
            ...
            results_composed.append(
                (
                    available_combinations[location_team][0],
                    available_combinations[location_team][1],
                    teams_types[index],
        ...
        for i in range(0, num_departments):
            print(
                ...
                i,
                " has a used capacity ",
                post_processing_package_bins_weights[i],
            )
            print("Unit ", i, " has a MAX capacity ", max_state[i])
