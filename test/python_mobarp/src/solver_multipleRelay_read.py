...
def create_vars(model_relay, NUM_FIXED_TX, NUM_TIME_STEPS, NUM_BANDS, NUM_MOBILE_TX, NUM_MOBILE_TX_LOCS):
    ...
    for i in range(NUM_FIXED_TX):
        ...
        for k in range(NUM_TIME_STEPS):
            ...
            for j in range(NUM_BANDS):
                ...
                temp.append(model_relay.NewBoolVar(name=f"x{i},{k},{j}"))
    ...
    for i_prime in range(NUM_MOBILE_TX):
        ...
        for n in range(NUM_MOBILE_TX_LOCS):
            ...
            temp.append(model_relay.NewBoolVar(name=f"r{i_prime},{n}"))
    ...
    for i_prime in range(NUM_MOBILE_TX):
        ...
        for k in range(NUM_TIME_STEPS):
            ...
            for j in range(NUM_BANDS):
                ...
                temp.append(model_relay.NewBoolVar(name=f"xr{i_prime},{k},{j}"))
            dec_vars.append(temp)
        ...
        for n in range(NUM_MOBILE_TX_LOCS):
            ...
            for k in range(NUM_TIME_STEPS):
                ...
                for j in range(NUM_BANDS):
                    ...
                    temp.append(model_relay.NewBoolVar(name=f"z{i_prime},{n},{k},{j}"))
                dec_vars.append(temp)
...
def create_capacity_constraints(model_relay, NUM_FIXED_TX, NUM_TIME_STEPS, NUM_BANDS, NUM_MOBILE_TX, dec_vars_sites, dec_vars_relay_broadcast, capacity_fixed, capacity_mobile):
    for i in range(NUM_FIXED_TX):
        for k in range(NUM_TIME_STEPS):
            ...
            model_relay.Add(sum(terms) <= capacity_fixed[i])
    ...
    for i_prime in range(NUM_MOBILE_TX): 
        for k in range(NUM_TIME_STEPS):
            ...
            model_relay.Add(sum(terms) <= capacity_mobile[i_prime])
        ...
        for n in range(NUM_MOBILE_TX_LOCS):
            for k in range(NUM_TIME_STEPS):
                for j in range(NUM_BANDS):
                    ...
                    model_relay.Add(dec_vars_aux[i_prime][n][k][j] <= dec_vars_relay_locs[i_prime][n]) #z_i_prime,n,k,j <= r_i_prime,n
                    ...
                    model_relay.Add(dec_vars_aux[i_prime][n][k][j] <= dec_vars_relay_broadcast[i_prime][k][j]) #z_i_prime,n,k,j <= x_i_prime,k,j
...
if __name__=="__main__":
    ...
    input_file_name = f"../../../data/inputs_mobarp_optimized_random_{type_sim}_cpsat_snr_threshold{snr_threshold}/set{set_number}"
    
    output_file_name = f"outputs_mobarp_optimized_random_{type_sim}_cpsat_snr_threshold{snr_threshold}/set{set_number}_ortools_singleThread_linearization_level2"
...
# while (index_sample<n_samples):
    ...
    # NUM_MOBILE_TX = 5 #np.random.randint(low = min_num_mobile_tx, high = max_num_mobile_tx+1, dtype = int) #np.random.randint(low = 1, high = max_num_mobile_tx+1, dtype = int) # fixed_size 
    # NUM_BANDS =  2*NUM_MOBILE_TX #np.random.randint(low = 5, high = max_band_value+1, dtype = int) #np.random.randint(low = 2, high = max_band_value+1, dtype = int) # np.minimum(fixed_size, max_band_value)
    ...
    # for filename in os.listdir(f"../../../data/{output_file_name}"):
    #     match = pattern.search(filename)
    #     if match:
    #         numbers.append(int(match.group(1)))
    # print(numbers)
    # exit()
    for test_file in range(0, n_samples):
        # if (test_file in numbers):
        #      continue
        
        input_file = os.path.join(input_file_name, "inputs_set"+str(test_file)+".pkl")
        ...
        NUM_FIXED_TX = input_load[0]
        NUM_MOBILE_TX = input_load[1]
        NUM_BANDS = input_load[2]
        NUM_TIME_STEPS = input_load[3]
        NUM_RX_LOCS = input_load[4]
        NUM_MOBILE_TX_LOCS = input_load[5]

        capacity_fixed = input_load[6].tolist()
        capacity_mobile = input_load[7].tolist()

        connectivity_fixed_tx = np.transpose(input_load[8], (0, 3, 2, 1)) #input is (i,j,k,l)

        connectivity_mobile_tx = np.transpose(input_load[9], (0, 3, 2, 1)) #input is (n,j,k,l)
        
        # start_time = time.time()
        model_relay = cp_model.CpModel()
        ...
        print(f"Test file: {test_file} & NUM_FIXED_TX: {NUM_FIXED_TX} & NUM_MOBILE_TX: {NUM_MOBILE_TX} & NUM_BANDS: {NUM_BANDS} & NUM_TIME_STEPS: {NUM_TIME_STEPS} & NUM_RX_LOCS: {NUM_RX_LOCS} & NUM_MOBILE_TX_LOCS: {NUM_MOBILE_TX_LOCS} & EXCLUDE_CAP_FLAG: {EXCLUDE_CAP_FLAG} & snr_threshold: {snr_threshold}")
        solver = cp_model.CpSolver()
        solver.parameters.num_search_workers = 1
        solver.parameters.linearization_level = 2
        solver.parameters.search_branching = cp_model.FIXED_SEARCH #FIXED_SEARCH #AUTOMATIC_SEARCH
        # solver.parameters.cp_model_presolve = True

        # params = mathopt.SolveParameters(enable_output=False, solution_pool_size=1)#, threads=1)
        # params.threads = 1
        # print(params.solver_specific_options)
        # params.solution_limit = 1
        # params.time_limit = timedelta(seconds=600)
        ...
        solver.parameters.max_time_in_seconds = 1200 #for set 2 #1200 for set 1
        start_time = time.time()
        solver.Solve(model_relay)
        # params.time_limit = timedelta(seconds=600)
        ...
        if solver.StatusName() == "OPTIMAL":
            # print(f"result.termination.reason: {result.termination.reason}")
            print(f"solver.StatusName(): {solver.StatusName()}")
            print(f"Test file: {test_file} & NUM_FIXED_TX: {NUM_FIXED_TX} & NUM_MOBILE_TX: {NUM_MOBILE_TX} & NUM_BANDS: {NUM_BANDS} & NUM_TIME_STEPS: {NUM_TIME_STEPS} & NUM_RX_LOCS: {NUM_RX_LOCS} & NUM_MOBILE_TX_LOCS: {NUM_MOBILE_TX_LOCS} & EXCLUDE_CAP_FLAG: {EXCLUDE_CAP_FLAG} & snr_threshold: {snr_threshold}")
            ...
            if (save_flag):
                ...
                results_composed = [objective_value, solver.StatusName(), solver.parameters.search_branching, cpsat_time]

                with open(f"../../../data/{output_file_name}/outputs_set{test_file}.pkl", "wb") as f:
