...
void extrinsic_output_package_update(vector<vector<impalib_type>> rExtrinsicOutputBin, vector<impalib_type> rOric2PackageM) {
    ...
    for (int bin_index = 0; bin_index < rExtrinsicOutputBin.size(); bin_index++) {
        ...
        transform(rExtrinsicOutputBin[bin_index].begin(), rExtrinsicOutputBin[bin_index].end(), ExtrinsicOutputPackage.begin(), ExtrinsicOutputPackage.begin(), std::plus<impalib_type>());
    }
