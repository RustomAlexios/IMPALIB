...
class DegreeConstraint:
    ...
    def __init__(self, NUM_NODES, num_edge_variables, edge_connections, FILTERING_FLAG, ALPHA):
    ...
    def degree_constraint_to_edge_ec_update(self, edge_ec_to_degree_constraint_m):
        degree_constraint_to_eq_constraint_m = np.zeros((self.num_edge_variables, self.num_nodes), dtype = np_impa_lib)
        for index_node in range(self.num_nodes):
            ...
            for index_edge_variable in range(len(connections)):
                remaining_connections = connections[:index_edge_variable]+connections[index_edge_variable+1:]
                degree_constraint_to_eq_constraint_m[connections[index_edge_variable], index_node] = -np.min(edge_ec_to_degree_constraint_m[remaining_connections,index_node])
