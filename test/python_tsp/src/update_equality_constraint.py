...
class EqualityConstraint:
    def __init__(self, NUM_NODES, num_edge_variables, cost_matrix, edge_connections, edge_degree_constraint_cost, AUGMENTATION_FLAG, FILTERING_FLAG, ALPHA):
    ...
    def edge_ec_to_degree_constraint_relaxed_graph_update(self, degree_constraint_to_eq_constraint_m):
    ...
    def edge_ec_to_subtour_constraints_update(self, degree_constraint_to_eq_constraint_m, subtour_constraints_to_edge_ec_m, delta_S_indices_list, cost_edge_variable):
        all_subtour_constraints = np.array(range(len(delta_S_indices_list)))
        if len(delta_S_indices_list) == 1:
            edge_ec_to_subtour_constraints_m = np.zeros((self.num_edge_variables), dtype=np_impa_lib)
            edge_ec_to_subtour_constraints_m[delta_S_indices_list[0]] = np.sum(degree_constraint_to_eq_constraint_m, axis=1)[delta_S_indices_list[0]] + cost_edge_variable[delta_S_indices_list[0]]
            ...
            for index_subtour_constraint in range(len(delta_S_indices_list)):
                remaining_subtour_constraints_indices = np.setdiff1d(all_subtour_constraints, index_subtour_constraint)
                sum_degree_constraint_to_eq_constraint_m = np.zeros(self.num_edge_variables)
                sum_degree_constraint_to_eq_constraint_m[delta_S_indices_list[index_subtour_constraint]] = np.sum(degree_constraint_to_eq_constraint_m[delta_S_indices_list[index_subtour_constraint],:], axis=1)
                ...
                sum_subtours[delta_S_indices_list[index_subtour_constraint]] = np.sum(np.array(subtour_constraints_to_edge_ec_m).T[:, remaining_subtour_constraints_indices], axis=1)[delta_S_indices_list[index_subtour_constraint]]
    ...
    def edge_ec_to_degree_constraint_augmented_graph_update(self, degree_constraint_to_eq_constraint_m, subtour_constraints_to_edge_ec_m):
        combined_subtour_constraints_to_edge_ec_m = np.sum(np.array(subtour_constraints_to_edge_ec_m).T, axis=1)
        flipped_degree_constraint_to_eq_constraint_m = self.flip_matrix(degree_constraint_to_eq_constraint_m)
    ...
    def flip_matrix(self, matrix):
        ...
        for l in range(len(self.edge_connections)):
            ...
            flipped_matrix[l, row] = matrix[l,col]; flipped_matrix[l, col] = matrix[l,row]
