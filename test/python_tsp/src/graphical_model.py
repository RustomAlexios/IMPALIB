...
class GraphicalModel:
    ...
    def __init__(self,NUM_ITERATIONS, NUM_NODES, SYMMETRIC_FLAG, AUGMENTATION_FLAG, EXACT_SOLVER_FLAG, LKH_SOLVER_FLAG, SIM_ANNEALING_FLAG, NUM_AUGMENTED_SAMPLES_PER_SUBTOUR_SIZE, RESET_FLAG, FILTERING_FLAG, ALPHA, THRESHOLD, RANDOM_TEST_FLAG):
    ...
    def initialize(self):
        ...
        self.model_eq_constraint = EqualityConstraint(num_nodes, num_edge_variables, cost_matrix, self.edge_connections, self.edge_degree_constraint_cost, self.augmentation_flag, self.filtering_flag, self.alpha)
        self.model_degree_constraint = DegreeConstraint(num_nodes, num_edge_variables, edge_connections, self.filtering_flag, self.alpha)
        self.outputs = OutputsImpa(num_nodes, num_edge_variables, cost_matrix, self.augmentation_flag)
        self.delta_S_indices_list = []
        if (self.augmentation_flag):
            self.model_subtour_constraint = SubtourEliminationConstraint(num_nodes, num_edge_variables, self.filtering_flag, self.alpha)
    ...
    def iterate_relaxed_graph(self):
    ...
    def pre_analysis(self):
        ...
        if (self.save_flag):
            self.results_composed.append((self.reset_flag, self.filtering_flag, self.alpha, self.cost_matrix, self.tour_impa, self.cost_impa, self.tour_lkh, self.cost_lkh, self.open_paths, self.runtime_impa, self.runtime_lkh))
        
            
    def iterate_augmented_graph(self):
        
        if (self.reset_flag):
            for i in range(len(self.edge_connections)):
                connection = self.edge_connections[i]
                cost = self.cost_matrix[connection[0], connection[1]]
        ...
        for iter in range(0, self.num_iterations):
            ...
            self.edge_ec_to_subtour_constraints_m = self.model_eq_constraint.edge_ec_to_subtour_constraints_update(self.model_degree_constraint.degree_constraint_to_eq_constraint_m, self.subtour_constraints_to_edge_ec_m, self.delta_S_indices_list, self.cost_edge_variable)
                
            self.subtour_constraints_to_edge_ec_m_dummy = self.model_subtour_constraint.subtour_constraints_to_edge_ec_update(self.edge_ec_to_subtour_constraints_m, self.delta_S_indices_list)

            self.subtour_constraints_to_edge_ec_m = self.model_subtour_constraint.process_filtering(iter)

            self.edge_ec_to_degree_constraint_m = self.model_eq_constraint.edge_ec_to_degree_constraint_augmented_graph_update(self.model_degree_constraint.degree_constraint_to_eq_constraint_m, self.subtour_constraints_to_edge_ec_m)
            
        extrinsic_output_edge_ec = self.outputs.extrinsic_output_edge_ec_augmented_graph_update(self.model_degree_constraint.degree_constraint_to_eq_constraint_m,self.subtour_constraints_to_edge_ec_m)
        intrinsic_out_edge_ec = extrinsic_output_edge_ec + self.cost_edge_variable
        self.intrinsic_out_edge_ec = intrinsic_out_edge_ec
    
    def find_closed_loop_new(self, graph, start, current, visited, path=[]):
        ...
        for node in graph[current]:
            if node not in visited:
                new_path = self.find_closed_loop_new(graph, start, node, visited.copy(), path)
                if (new_path):
                    return new_path
    ...
    def get_closed_loops(self):
        ...
        for start_node, end_node in graph.items():
            if (start_node in self.visited_nodes):
                ...
                for j in range(len(end_node)):
                    closed_loop = self.find_closed_loop_new(graph, start_node, end_node[j], set())
        ...
        for i, list_1 in enumerate(loops_list):
            if (list_1 is None):
            ...
            for j, list_2 in enumerate(loops_list[i+1:], start=i+1):
                if list_2 is None:
        ...
        for sublist in new_loops_list:
            sublist.insert(0, sublist.pop(-1))
        ...
        if (len(self.open_paths_dummy)):
            ...
            for index, element in enumerate(open_paths_dummy):
                size = len(element)
                for l, other_element in enumerate(open_paths_dummy[:index] + open_paths_dummy[index+1:]):
    ...
    def subtour_elimination_constraints_analysis(self):
        ...
        if (not loops_list):
            ...
            for sublist in loops_list:
                ...
                pairs.append([sublist[-1], sublist[0]])
                ...
                delta_S_indices = [i for i, connection in enumerate(self.edge_connections) if not tuple(connection) in list(itertools.permutations(sublist, 2)) and connection[0] in sublist]
    ...
    def hard_decision_analysis(self):
        ...
        print('selected_edges: ', self.selected_edges.tolist())
        print('cost_impa: ', self.cost_impa)
