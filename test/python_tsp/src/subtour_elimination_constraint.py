...
class SubtourEliminationConstraint:
    ...
    def __init__(self, NUM_NODES, num_edge_variables, FILTERING_FLAG, ALPHA):
    ...
    def subtour_constraints_to_edge_ec_update(self, edge_ec_to_subtour_constraints_m, delta_S_indices_list):
        ...
        for index_subtour_constraint in range(len(delta_S_indices_list)):
            ...
            for index_edge_variable in range(len(delta_S_indices)):
                remaining_connections = delta_S_indices[:index_edge_variable]+delta_S_indices[index_edge_variable+1:] 
                subtour_constraints_to_edge_ec_m[delta_S_indices[index_edge_variable], index_subtour_constraint] = -np.maximum(min(np.array(edge_ec_to_subtour_constraints_m).T[remaining_connections,index_subtour_constraint]), zero_value)
        self.subtour_constraints_to_edge_ec_m = subtour_constraints_to_edge_ec_m.T.tolist()
