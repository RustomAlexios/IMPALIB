...
def ut_model_graph(ut_name, n_nodes, filt_flag, alpha, threshold, random_test_flag, n_iter , sym_flag):
    ...
    if (ut_name == "IterateRelaxedGraph"):   
        ...
        ModelIMPA = model_graph.GraphicalModel(N_ITER, N_NODES, SYM_FLAG, AUGM_FLAG, EXACT_SOLVER_FLAG, LKH_SOLVER_FLAG, SIM_ANNEALING_FLAG, NUM_AUGMENTED_SAMPLES_PER_SUBTOUR_SIZE, RESET_FLAG, FILT_FLAG, ALPHA, THRESHOLD, RANDOM_TEST_FLAG)
        ...
        f_input4 = os.getcwd() + '/../ut_inputs/ut_GraphicalModel/ut_'+ ut_name+'/edge_ec_to_degree_constraint_m_pure.npy'
        np.save(f_input4, np.array(ModelIMPA.edge_ec_to_degree_constraint_m, dtype=np_impa_lib).flatten())     
        
        f_input5 = os.getcwd() + '/../ut_inputs/ut_GraphicalModel/ut_'+ ut_name+'/edge_degree_constraint_cost_pure.npy'
