...
extern "C" void WrapperKcMwm(const int NUM_ITERATIONS, const int NUM_DEPARTMENTS, const int NUM_TEAMS, const int NUM_PROJECTS, 
...
extern "C" void WrapperTsp(const int NUM_ITERATIONS, const int NUM_NODES, const int NUM_EDGE_VARIABLES, const bool AUGMENTATION_FLAG, const bool RESET_FLAG, 
                const bool FILTERING_FLAG, const impalib_type ALPHA, const int *pEDGE_CONNECTIONS_PY, const impalib_type *pCOST_EDGE_VARIABLE_PY, const impalib_type *pCOST_MATRIX_PY, 
                ...
                int *pNum_augmentations, int *pTour_impa, impalib_type *pCost_impa, bool *pNo_improv_sol_count_exc_flag, bool *pNo_cons_loops_count_exc_flag, int *pSelected_edges, int *pSelected_edges_size, \
                bool *pSol_osc_count_exc_flag, int *pSubtour_paths, int *pSubtour_paths_size, const int MAX_COUNT, \
                    int *pNum_added_constraints, const int MAX_AUGM_COUNT){
    ...
    if (!model_graph.subtourConstraintsSatisfiedFlag && AUGMENTATION_FLAG){
        if (model_graph.delta_S_indices_list.size() >0){
                vector<vector<impalib_type>> temp(model_graph.delta_S_indices_list.size(), vector<impalib_type>(NUM_EDGE_VARIABLES, zero_value));
    ...
    while (!model_graph.subtourConstraintsSatisfiedFlag && AUGMENTATION_FLAG && !model_graph.tourImpaFlag){
        ...
        if (model_graph.noConsClosedLoopsCountExcFlag_ || model_graph.noImprovSolCountExcFlag_ || model_graph.solOscCountExcFlag_){
            cout<<"model_graph.noConsClosedLoopsCountExcFlag_: "<<model_graph.noConsClosedLoopsCountExcFlag_<<endl;
            cout<<"model_graph.noImprovSolCountExcFlag_: "<<model_graph.noImprovSolCountExcFlag_<<endl;
            cout<<"model_graph.solOscCountExcFlag_: "<<model_graph.solOscCountExcFlag_<<endl;
            break;
        }
        if (model_graph.costImpa_ == zero_value){
            cout<<"Cost is zero"<<endl;
            cout<<"Possibly Nan"<<endl;
            exit(0);
        }
    ...
    copy(model_graph.outputs.ExtrinsicOutputEdgeEc.begin(), model_graph.outputs.ExtrinsicOutputEdgeEc.begin() + NUM_EDGE_VARIABLES, pExtrinsic_output_edge_ec);
    *pNum_augmentations = model_graph.numAugmentations_;
    *pNum_added_constraints = model_graph.subtourConstraints2EdgeEcM.size();
    copy(model_graph.tourImpa_.begin(), model_graph.tourImpa_.begin() + model_graph.tourImpa_.size(), pTour_impa);
    ...
    *pSol_osc_count_exc_flag = model_graph.solOscCountExcFlag_;
    ...
    vector<int> flattened_selected_edges = accumulate(model_graph.selectedEdges_.begin(), model_graph.selectedEdges_.end(), vector<int>{}, [](vector<int>& acc, const vector<int>& inner) {
            acc.insert(acc.end(), inner.begin(), inner.end());return acc; });

    copy(flattened_selected_edges.begin(), flattened_selected_edges.begin() + flattened_selected_edges.size(), pSelected_edges);
    *pSelected_edges_size = flattened_selected_edges.size();
    
    vector<int> flattened_closed_paths = accumulate(model_graph.subtourPaths_.begin(), model_graph.subtourPaths_.end(), vector<int>{}, [](vector<int>& acc, const vector<int>& inner) {
            acc.insert(acc.end(), inner.begin(), inner.end());return acc; });
    
    copy(flattened_closed_paths.begin(), flattened_closed_paths.begin() + flattened_closed_paths.size(), pSubtour_paths);
    copy(model_graph.closedPathsSize_.begin(), model_graph.closedPathsSize_.begin() + model_graph.closedPathsSize_.size(), pSubtour_paths_size);
}