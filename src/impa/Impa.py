...
class GraphicalModelMOBARP:
    ...
    def initialize(self):
        ...
        self.fixed_x_eq_const_to_fixed_capac_const_m = np.zeros_like(fixed_x_costs, dtype = np_impa_lib)
        self.mobile_x_eq_const_to_mobile_capac_const_m = np.zeros_like(mobile_x_costs, dtype = np_impa_lib)
        ...
        # self.auxiliary_const_to_mobile_x_eq_const_m = np.zeros((self.mobile_x_costs.size, self.num_mobile_tx_locs), dtype= np_impa_lib)
        self.r_eq_const_to_auxiliary_const_m = np.concatenate([self.r_costs.flatten()[..., np.newaxis]]*self.num_bands*self.num_time_steps, axis=1)
        # self.z_eq_const_to_auxiliary_const_m = np.zeros((self.mobile_x_costs.size, self.num_mobile_tx_locs), dtype= np_impa_lib)
        # self.mobile_loc_eq_const_to_r_eq_const_m= np.zeros((self.num_mobile_tx, self.num_mobile_tx_locs), dtype=np_impa_lib)
        ...
        self.r_eq_const_to_auxiliary_const_m *=reshaped_connectivity_mobile_tx_aux
        # self.set_cover_ineq_const_to_z_eq_const_m = np.zeros((self.num_time_steps, self.num_rx_locs, self.num_mobile_tx_locs, self.num_bands*self.num_mobile_tx), dtype=np_impa_lib)
        # self.set_cover_ineq_const_to_fixed_x_eq_const_m = np.zeros((self.num_time_steps, self.num_rx_locs, self.num_fixed_tx*self.num_bands), dtype=np_impa_lib)
    ...
    def run_impa(self):
        ...
        (
            fixed_x_eq_const_to_fixed_capac_const_m_flatten_p, #c_impa_lib_type_p
            mobile_x_eq_const_to_mobile_capac_const_m_flatten_p, #c_impa_lib_type_p
            r_eq_const_to_auxiliary_const_m_flatten_p, #c_impa_lib_type_p
            fixed_x_costs_flatten_p, #c_impa_lib_type_p
            mobile_x_costs_flatten_p, #c_impa_lib_type_p
            z_costs_flatten_p, #c_impa_lib_type_p
            r_costs_flatten_p, #c_impa_lib_type_p
            connectivity_fixed_tx_flatten_p, #c_int_p
            connectivity_mobile_tx_flatten_p, #c_int_p
            extrinsic_fixed_x_p, #c_impa_lib_type_p
            extrinsic_mobile_x_p, #c_impa_lib_type_p
            extrinsic_z_p, #c_impa_lib_type_p
            extrinsic_r_p, #c_impa_lib_type_p
            fixed_capac_constraints_flatten_p, #c_int_p
            mobile_capac_constraints_flatten_p, #c_int_p
            conx_mob_tx_per_num_mob_tx_locs_flatten_p, #c_int_p
            conx_fixed_tx_per_num_rx_locs_flatten_p,  #c_int_p
            conx_mob_tx_rx_flatten_p, #c_int_p
        ) = self.process_inputs_ctypes()
        
        WrapperMOBARP(
            np.int32(self.num_iterations),
            np.int32(self.num_fixed_tx),
            np.int32(self.num_mobile_tx),
            np.int32(self.num_bands),
            np.int32(self.num_time_steps),
            np.int32(self.num_rx_locs),
            np.int32(self.num_mobile_tx_locs),
            np_impa_lib(self.alpha),
            self.filtering_flag,
            self.exclude_cap_flag,
            fixed_x_eq_const_to_fixed_capac_const_m_flatten_p,
            mobile_x_eq_const_to_mobile_capac_const_m_flatten_p,
            r_eq_const_to_auxiliary_const_m_flatten_p,
            fixed_x_costs_flatten_p,
            mobile_x_costs_flatten_p,
            z_costs_flatten_p,
            r_costs_flatten_p,
            connectivity_fixed_tx_flatten_p,
            connectivity_mobile_tx_flatten_p,
            extrinsic_fixed_x_p,
            extrinsic_mobile_x_p,
            extrinsic_z_p,
            extrinsic_r_p,
            fixed_capac_constraints_flatten_p,
            mobile_capac_constraints_flatten_p,
            conx_mob_tx_per_num_mob_tx_locs_flatten_p,
            conx_fixed_tx_per_num_rx_locs_flatten_p,
            conx_mob_tx_rx_flatten_p
        )
        
        extrinsic_fixed_x = list(extrinsic_fixed_x_p.__dict__.values())[0]
        extrinsic_mobile_x = list(extrinsic_mobile_x_p.__dict__.values())[0]
        extrinsic_r = list(extrinsic_r_p.__dict__.values())[0]
        extrinsic_z = list(extrinsic_z_p.__dict__.values())[0].reshape((self.num_bands*self.num_mobile_tx*self.num_time_steps, self.num_mobile_tx_locs))
        
        # print("extrinsic_fixed_x: \n", extrinsic_fixed_x)
        # print("extrinsic_mobile_x: \n", extrinsic_mobile_x)
        # print("extrinsic_r: \n", extrinsic_r)
        # print("extrinsic_z: \n", extrinsic_z)
    ...
    def process_inputs_ctypes(self):
        ...
        fixed_x_eq_const_to_fixed_capac_const_m_flatten = self.fixed_x_eq_const_to_fixed_capac_const_m.flatten().astype(np_impa_lib)
        fixed_x_eq_const_to_fixed_capac_const_m_flatten_p = fixed_x_eq_const_to_fixed_capac_const_m_flatten.ctypes.data_as(c_impa_lib_type_p)
        
        mobile_x_eq_const_to_mobile_capac_const_m_flatten = self.mobile_x_eq_const_to_mobile_capac_const_m.flatten().astype(np_impa_lib)
        mobile_x_eq_const_to_mobile_capac_const_m_flatten_p = mobile_x_eq_const_to_mobile_capac_const_m_flatten.ctypes.data_as(c_impa_lib_type_p)
        
        r_eq_const_to_auxiliary_const_m_flatten = self.r_eq_const_to_auxiliary_const_m.flatten().astype(np_impa_lib)
        r_eq_const_to_auxiliary_const_m_flatten_p = r_eq_const_to_auxiliary_const_m_flatten.ctypes.data_as(c_impa_lib_type_p)
        
        fixed_x_costs_flatten = self.fixed_x_costs.flatten().astype(np_impa_lib)
        fixed_x_costs_flatten_p = fixed_x_costs_flatten.ctypes.data_as(c_impa_lib_type_p)
        
        mobile_x_costs_flatten = self.mobile_x_costs.flatten().astype(np_impa_lib)
        mobile_x_costs_flatten_p = mobile_x_costs_flatten.ctypes.data_as(c_impa_lib_type_p)
        
        z_costs_flatten = self.z_costs.flatten().astype(np_impa_lib)
        z_costs_flatten_p = z_costs_flatten.ctypes.data_as(c_impa_lib_type_p)
        
        r_costs_flatten = self.r_costs.flatten().astype(np_impa_lib)
        r_costs_flatten_p = r_costs_flatten.ctypes.data_as(c_impa_lib_type_p)
        
        connectivity_fixed_tx_flatten = self.connectivity_fixed_tx.flatten().astype(np.int32)
        connectivity_fixed_tx_flatten_p = connectivity_fixed_tx_flatten.ctypes.data_as(c_int_p)
        
        connectivity_mobile_tx_flatten = self.connectivity_mobile_tx.flatten().astype(np.int32)
        connectivity_mobile_tx_flatten_p = connectivity_mobile_tx_flatten.ctypes.data_as(c_int_p)
        
        extrinsic_fixed_x = np.zeros_like(self.fixed_x_costs.flatten(), dtype = np_impa_lib)
        extrinsic_fixed_x = np.array(extrinsic_fixed_x).flatten().astype(np_impa_lib)
        extrinsic_fixed_x_p = extrinsic_fixed_x.ctypes.data_as(c_impa_lib_type_p)
        
        extrinsic_mobile_x = np.zeros_like(self.mobile_x_costs.flatten(), dtype = np_impa_lib)
        extrinsic_mobile_x = np.array(extrinsic_mobile_x).flatten().astype(np_impa_lib)
        extrinsic_mobile_x_p = extrinsic_mobile_x.ctypes.data_as(c_impa_lib_type_p)
        
        extrinsic_z = np.zeros_like(self.conx_mob_tx_per_num_mob_tx_locs.flatten(), dtype = np_impa_lib)
        extrinsic_z = np.array(extrinsic_z).flatten().astype(np_impa_lib)
        extrinsic_z_p = extrinsic_z.ctypes.data_as(c_impa_lib_type_p)
        
        extrinsic_r = np.zeros((self.num_mobile_tx*self.num_mobile_tx_locs), dtype = np_impa_lib)
        extrinsic_r = np.array(extrinsic_r).flatten().astype(np_impa_lib)
        extrinsic_r_p = extrinsic_r.ctypes.data_as(c_impa_lib_type_p)
        
        fixed_capac_constraints_flatten = self.fixed_capac_constraints.flatten().astype(np.int32)
        fixed_capac_constraints_flatten_p = fixed_capac_constraints_flatten.ctypes.data_as(c_int_p)
        
        mobile_capac_constraints_flatten = self.mobile_capac_constraints.flatten().astype(np.int32)
        mobile_capac_constraints_flatten_p = mobile_capac_constraints_flatten.ctypes.data_as(c_int_p)
        
        conx_mob_tx_per_num_mob_tx_locs_flatten = self.conx_mob_tx_per_num_mob_tx_locs.flatten().astype(np.int32)
        conx_mob_tx_per_num_mob_tx_locs_flatten_p = conx_mob_tx_per_num_mob_tx_locs_flatten.ctypes.data_as(c_int_p)
        
        conx_fixed_tx_per_num_rx_locs_flatten = self.conx_fixed_tx_per_num_rx_locs.flatten().astype(np.int32)
        conx_fixed_tx_per_num_rx_locs_flatten_p = conx_fixed_tx_per_num_rx_locs_flatten.ctypes.data_as(c_int_p)
        
        conx_mob_tx_rx_flatten = self.conx_mob_tx_rx.flatten().astype(np.int32)
        conx_mob_tx_rx_flatten_p = conx_mob_tx_rx_flatten.ctypes.data_as(c_int_p)
        
        # print(self.fixed_capac_constraints)
        # print(self.mobile_capac_constraints)
        return (
            fixed_x_eq_const_to_fixed_capac_const_m_flatten_p, #c_impa_lib_type_p
            mobile_x_eq_const_to_mobile_capac_const_m_flatten_p, #c_impa_lib_type_p
            r_eq_const_to_auxiliary_const_m_flatten_p, #c_impa_lib_type_p
