...
def ut_degree_constraint(ut_name, n_edge_variables, n_nodes, edge_connections, filt_flag, alpha):
    ...
    f_input1 = (
        os.getcwd() + "/../ut_inputs/ut_DegreeConstraint/ut_" + ut_name + "/edge_ec_to_degree_constraint_m_pure.npy"
    )
    ...
    f_input2 = os.getcwd() + "/../ut_inputs/ut_DegreeConstraint/ut_" + ut_name + "/edge_connections_pure.npy"
    np.save(f_input2, np.array(EDGE_CONNECTIONS, dtype=np.int32).flatten())

    f_input_alpha = os.getcwd() + "/../ut_inputs/ut_DegreeConstraint/ut_" + ut_name + "/alpha.npy"
    np.save(f_input_alpha, ALPHA)

    model_degree_constraint = degree_constraint.DegreeConstraint(
        N_NODES, N_EDGE_VARIABLES, EDGE_CONNECTIONS, FILT_FLAG, ALPHA
    )
    ...
    if ut_name == "DegreeConstraint2EdgeEcUpdate":
        ...
        f_output_path = (
            os.getcwd()
            + "/../ut_results/ut_DegreeConstraint/ut_"
            + ut_name
