...
def ut_io(ut_name, n_edge_variables, n_nodes, n_subtours, edge_connections):
    ...
    f_input1 = (
        os.getcwd() + "/../ut_inputs/ut_InputOutput/ut_" + ut_name + "/degree_constraint_to_eq_constraint_m_pure.npy"
    )
    ...
    if ut_name == "ExtrinsicOutputEdgeEcRelaxedGraphUpdate":
        extrinsic_output_edge_ec_relaxed_graph_pure = outputs.extrinsic_output_edge_ec_relaxed_graph_update(
            degree_constraint_to_eq_constraint_m_pure
        )
        f_output_path = (
            os.getcwd() + "/../ut_results/ut_InputOutput/ut_" + ut_name + "/extrinsic_output_edge_ec_relaxed_graph_pure"
        )
        ...
        delta_S_indices_list = [
            random.sample(range(N_EDGE_VARIABLES), k=random.randint(2, N_NODES))
            for _ in range(delta_S_indices_list_length)
        ]
        ...
        f_input2 = (
            os.getcwd() + "/../ut_inputs/ut_InputOutput/ut_" + ut_name + "/subtour_constraints_to_edge_ec_m_pure.npy"
        )
        np.save(f_input2, subtour_constraints_to_edge_ec_m_pure.flatten())

        extrinsic_output_edge_ec_augmented_graph_pure = outputs.extrinsic_output_edge_ec_augmented_graph_update(
            degree_constraint_to_eq_constraint_m_pure, subtour_constraints_to_edge_ec_m_pure
        )
        f_output_path = (
            os.getcwd()
            + "/../ut_results/ut_InputOutput/ut_"
            + ut_name
            + "/extrinsic_output_edge_ec_augmented_graph_pure"
        )
        output_file_python_pure = open(f_output_path, "wb")
