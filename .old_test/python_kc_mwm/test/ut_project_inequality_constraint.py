...
def ut_project_ineq_constraint(ut_name, n_departments, n_teams, n_projects, unbalanced_flag):
    ...
    project_ineq_constraint = ineq_constraint_project.InequalityConstraint(
        N_DEPARTMENTS, N_TEAMS, N_PROJECTS, unbalanced_flag
    )

    f_input_path = (
        os.getcwd() + "/../ut_inputs/ut_projectIneqConstraint/ut_" + ut_name + "/eq_constraint_to_project_m_pure.npy"
    )
    ...
    if ut_name == "ProjectInequalityConstraintUpdate":
        f_output_path = (
            os.getcwd() + "/../ut_results/ut_projectIneqConstraint/ut_" + ut_name + "/project_to_eq_constraint_m_pure"
        )
        project_to_eq_constraint_m_pure = project_ineq_constraint.project_inequality_constraint_update(
