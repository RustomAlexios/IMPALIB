    ...
    def exactly_one_update(self, A, cols):
        ...
        if cols:
            ...
            for j in range(1, A.shape[0] - 1):
                lower_bound = np.min(A[:j, :], axis=0)
                upper_bound = np.min(A[j + 1 :, :], axis=0)
                B[j] = self.constraint_rule(np.minimum(lower_bound, upper_bound))
            B[A.shape[0] - 1] = self.constraint_rule(np.min(A[: A.shape[0] - 1, :], axis=0))
        else:
            B[:, 0] = self.constraint_rule(np.min(A[:, 1:], axis=1))
            for j in range(1, A.shape[1] - 1):
                lower_bound = np.min(A[:, :j], axis=1)
                upper_bound = np.min(A[:, j + 1 :], axis=1)
                B[:, j] = self.constraint_rule(np.minimum(lower_bound, upper_bound))
