...
int main()
{
    ...
    array<int, N_DEPARTMENTS> non_zero_weight_indices_sizes = {4, 4};
    ...
    // int max_size_non_zero_weight = *max_element(pNON_ZERO_WEIGHT_INDICES_SIZES , pNON_ZERO_WEIGHT_INDICES_SIZES +
    // N_DEPARTMENTS);

    GraphicalModelKcMwm model_graph(N_DEPARTMENTS, N_TEAMS, N_PROJECTS, max_size_non_zero_weight, N_ITER, FILT_FLAG,
                                    ALPHA);

    array<impalib_type, N_TEAMS>             reward_team    = {-22, -31, -68, -39, -84}; // cost of activating a team
    array<impalib_type, N_PROJECTS *N_TEAMS> reward_project = {
        44, 1,  41, 10, 3, // cost of assigning a team to a project
        7,  17, 56, 98, 63};
