...
inline void OrInequalityConstraint::oric_to_project_eq_constraint_update(const vector<vector<impalib_type>> &eq2OricM,
                                                                  const vector<impalib_type>         &team2OricM,
                                                                  vector<vector<impalib_type>> &oric2EqM,
                                                                  ...
                                                                  const vector<vector<impalib_type>> &rewards) const
{
    ...
    for (int team = 0; team < numTeams_; team++)
    {
        ...
        for (int stage = 0; stage < numProjects_; stage++)
        {
            ...
            forward[stage + 1][1] =
                min(forward[stage][1], forward[stage][0]
                                                                       + eq2OricM[stage][team]
                                                                       + team2OricM[team]);
        }
        ...
        for (int stage = numProjects_ - 1; stage >= 0; stage--)
        {
            backward[stage][0] =
                min(backward[stage + 1][0],
                    backward[stage + 1][1] + eq2OricM[stage][team]
                        + team2OricM[team]);
...
inline void OrInequalityConstraint::oric_to_team_update(const vector<vector<impalib_type>> &eq2OricM,
                                                 vector<impalib_type>         &oric2TeamM)
{
