...
class EqualityConstraint {
   ...
   public:
    EqualityConstraint(const int N_DEPARTMENTS, const int N_TEAMS, const int N_PROJECTS)
        : nProj_(N_PROJECTS), nTeams_(N_TEAMS), nDept_(N_DEPARTMENTS), doFilter_(false), alpha_(zero_value), nNodes_(0), nEdgeVars_(0), nVars_(0), nConstraints_(0), k_(0){};

    EqualityConstraint(const int NUM_NODES, const int NUM_EDGE_VARIABLES, const bool FILTERING_FLAG, const impalib_type ALPHA)
        : nProj_(0), nTeams_(0), nDept_(0), doFilter_(FILTERING_FLAG), alpha_(ALPHA), nNodes_(NUM_NODES), nEdgeVars_(NUM_EDGE_VARIABLES), nVars_(0), nConstraints_(0), k_(0){};

    EqualityConstraint(const int NUM_VARIABLES, const int NUM_CONSTRAINTS, const int K_VARIABLE, const bool FILTERING_FLAG, const impalib_type ALPHA)
        : nProj_(0), nTeams_(0), nDept_(0), doFilter_(FILTERING_FLAG), alpha_(ALPHA), nNodes_(0), nEdgeVars_(0), nVars_(NUM_VARIABLES), nConstraints_(NUM_CONSTRAINTS), k_(K_VARIABLE){};
...
inline void EqualityConstraint::team_eq_constraint_to_oric_update(vector<vector<impalib_type>> &extrinsicOut, vector<impalib_type> &team2OricM, vector<impalib_type> &rewards) const {
    vector<impalib_type> intermediate_team_to_oric_m(nTeams_, 0);

    for (int department = 0; department < extrinsicOut.size(); department++) {
        transform(extrinsicOut[department].begin(), extrinsicOut[department].end(), intermediate_team_to_oric_m.begin(), intermediate_team_to_oric_m.begin(), std::plus<impalib_type>());
    }
    transform(intermediate_team_to_oric_m.begin(), intermediate_team_to_oric_m.end(), rewards.begin(), team2OricM.begin(), std::plus<impalib_type>());
}
...
inline void EqualityConstraint::project_eq_constraint_to_oric_update(vector<vector<impalib_type>> &project2EqM, vector<vector<impalib_type>> &eq2OricM, vector<vector<impalib_type>> &rewards) {
    for (int project = 0; project < project2EqM.size(); project++) {
        transform(project2EqM[project].begin(), project2EqM[project].end(), rewards[project].begin(), eq2OricM[project].begin(), std::plus<impalib_type>());
    }
}
...
inline void EqualityConstraint::edge_ec_to_degree_constraint_relaxed_graph_update(const vector<vector<int>> &connections, vector<vector<impalib_type>> &cost,
                                                                                  const vector<vector<impalib_type>> &deg2EqM, vector<vector<impalib_type>> &eq2DegM) const {
    ...
    for (int edge = 0; edge < nEdgeVars_; edge++) {
        transform(deg2EqM_flipped[edge].begin(), deg2EqM_flipped[edge].end(), cost[edge].begin(), eq2DegM[edge].begin(), plus<impalib_type>());
    }
}
...
inline vector<vector<impalib_type>> EqualityConstraint::edge_ec_to_subtour_constraints_update(const vector<vector<int>> &deltaS, const vector<impalib_type> &costs,
                                                                                              const vector<vector<impalib_type>> &deg2EqM, const vector<vector<impalib_type>> &subtour2EqM,
                                                                                              const vector<vector<int>> &connections) const {
    ...
    if (deltaS.size() == 1) {
        ...
        for (size_t edge = 0; edge < nEdgeVars_; ++edge) {
            deg2EqM_sum[edge] = deg2EqM[edge][connections[edge][0]] + deg2EqM[edge][connections[edge][1]];
        }

        for (size_t i = 0; i < deltaS[0].size(); i++) {
            edge_ec_to_subtour_constraints_m[deltaS[0][i]] = deg2EqM_sum[deltaS[0][i]] + costs[deltaS[0][i]];
        }
        ...
        for (const auto &row : subtour2EqM) {
            transform(combined_subtour_constraints_to_edge_ec_m.begin(), combined_subtour_constraints_to_edge_ec_m.end(), row.begin(), combined_subtour_constraints_to_edge_ec_m.begin(),
                      std::plus<impalib_type>());
        }

        vector<impalib_type> combined_degree_constraint_to_eq_constraint_m(nEdgeVars_, zero_value);
        for (size_t edge = 0; edge < nEdgeVars_; ++edge) {
            combined_degree_constraint_to_eq_constraint_m[edge] = deg2EqM[edge][connections[edge][0]] + deg2EqM[edge][connections[edge][1]];
        }

        for (size_t subtour = 0; subtour < deltaS.size(); subtour++) {
            vector<impalib_type> edge_ec_to_subtour_constraints_m(nEdgeVars_, zero_value);

            for (size_t i = 0; i < deltaS[subtour].size(); i++) {
                edge_ec_to_subtour_constraints_m[deltaS[subtour][i]] = combined_subtour_constraints_to_edge_ec_m[deltaS[subtour][i]] +
                                                                       combined_degree_constraint_to_eq_constraint_m[deltaS[subtour][i]] + costs[deltaS[subtour][i]] -
                                                                       subtour2EqM[subtour][deltaS[subtour][i]];
            }
            eq2SubtourM.push_back(edge_ec_to_subtour_constraints_m);
        }
    }
...
inline void EqualityConstraint::edge_ec_to_degree_constraint_augmented_graph_update(const vector<vector<impalib_type>> &deg2EqM, const vector<vector<impalib_type>> &subtour2EqM,
                                                                                    const vector<vector<int>> &connections, const vector<vector<impalib_type>> &cost,
                                                                                    vector<vector<impalib_type>> &eq2DegreeM) const {
    vector<impalib_type> subtour2EqM_sum(nEdgeVars_, zero_value);
    for (const auto &row : subtour2EqM) {
        transform(subtour2EqM_sum.begin(), subtour2EqM_sum.end(), row.begin(), subtour2EqM_sum.begin(), std::plus<impalib_type>());
    }

    for (size_t i = 0; i < connections.size(); i++) {
        // Update the message for the first node of the edge
        eq2DegreeM[i][connections[i][0]] = subtour2EqM_sum[i] + deg2EqM[i][connections[i][1]] + cost[i][connections[i][0]];

        // Update the message for the second node of the edge
        eq2DegreeM[i][connections[i][1]] = subtour2EqM_sum[i] + deg2EqM[i][connections[i][0]] + cost[i][connections[i][1]];
    }
}
...
inline void EqualityConstraint::variable_ec_to_ksat_constraint_update(const vector<vector<impalib_type>> &ksat2EqM, vector<vector<impalib_type>> &var2KsatM, vector<int> &used,
                                                                      const vector<impalib_type> &costs, const vector<vector<int>> &connections) const {
    ...
    for_each(used.begin(), used.end(), [&](int n) { used_incoming_metrics_cost[n] = costs[n]; });
