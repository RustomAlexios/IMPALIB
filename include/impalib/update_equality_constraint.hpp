...
class EqualityConstraintTsp{
    ...
    public:
        void edge_ec_to_degree_constraint_relaxed_graph_update(vector<vector<int>>&, vector<vector<impalib_type>>&, vector<vector<impalib_type>>&, vector<vector<impalib_type>>&);
        void flip_matrix(vector<vector<impalib_type>>&, vector<vector<int>>&, vector<vector<impalib_type>>&);
        vector<vector<impalib_type>> edge_ec_to_subtour_constraints_update(vector<vector<int>>&, vector<impalib_type>&, vector<vector<impalib_type>>&, vector<vector<impalib_type>>&, vector<vector<int>>&);
        void edge_ec_to_degree_constraint_augmented_graph_update(vector<vector<impalib_type>>&, vector<vector<impalib_type>>&, vector<vector<int>>&, vector<vector<impalib_type>>&, vector<vector<impalib_type>>&);
        

    EqualityConstraintTsp(const int NUM_NODES, const int NUM_EDGE_VARIABLES, const bool FILTERING_FLAG, const impalib_type ALPHA);
};
...
void EqualityConstraintTsp::edge_ec_to_degree_constraint_relaxed_graph_update(vector<vector<int>>& rEdgeConnections, vector<vector<impalib_type>>& rEdgeDegreeConstraintCost, vector<vector<impalib_type>>& rDegreeConstraint2EqConstraintM, vector<vector<impalib_type>>& rEdgeEc2DegreeConstraintM){
...
vector<vector<impalib_type>> EqualityConstraintTsp::edge_ec_to_subtour_constraints_update(vector<vector<int>>& rDeltaSIndicesList, vector<impalib_type>& rCostEdgeVaribale, vector<vector<impalib_type>>& rDegreeConstraint2EqConstraintM, vector<vector<impalib_type>>& rSubtourConstraints2EdgeEcM, \
                                                                                    vector<vector<int>>& rEdgeConnections){
    ...
    vector<vector<impalib_type>> edge_ec_to_subtour_constraints_list;
        ...
        if (rDeltaSIndicesList.size() == 1) {
            ...
            for (size_t i = 0; i < rDeltaSIndicesList[0].size(); i++){
                    edge_ec_to_subtour_constraints_m[rDeltaSIndicesList[0][i]] = combined_degree_constraint_to_eq_constraint_m[rDeltaSIndicesList[0][i]]\
            ...
            for (const auto& row : rSubtourConstraints2EdgeEcM) {
                transform(combined_subtour_constraints_to_edge_ec_m.begin(), combined_subtour_constraints_to_edge_ec_m.end(), row.begin(), combined_subtour_constraints_to_edge_ec_m.begin(), std::plus<impalib_type>());
            }
            ...
            for (size_t index_subtour_constraint = 0; index_subtour_constraint < rDeltaSIndicesList.size(); index_subtour_constraint++) {
                ...
                for (size_t i = 0; i < rDeltaSIndicesList[index_subtour_constraint].size(); i++){
                    edge_ec_to_subtour_constraints_m[rDeltaSIndicesList[index_subtour_constraint][i]] = combined_subtour_constraints_to_edge_ec_m[rDeltaSIndicesList[index_subtour_constraint][i]] \
...
void EqualityConstraintTsp::edge_ec_to_degree_constraint_augmented_graph_update(vector<vector<impalib_type>>& rDegreeConstraint2EqConstraintM, vector<vector<impalib_type>>& rSubtourConstraints2EdgeEcM, \
                                                                            vector<vector<int>>& rEdgeConnections, vector<vector<impalib_type>>& rEdgeDegreeConstraintCost, vector<vector<impalib_type>>& rEdgeEc2DegreeConstraintM){
        ...
        for (size_t i = 0; i < rEdgeConnections.size(); i++) {
            ...
            rEdgeEc2DegreeConstraintM[i][rEdgeConnections[i][0]] = combined_subtour_constraints_to_edge_ec_m[i] +
                            rDegreeConstraint2EqConstraintM[i][rEdgeConnections[i][1]] +
                            rEdgeDegreeConstraintCost[i][rEdgeConnections[i][0]];

            rEdgeEc2DegreeConstraintM[i][rEdgeConnections[i][1]] = combined_subtour_constraints_to_edge_ec_m[i] +
...
void EqualityConstraintTsp::flip_matrix(vector<vector<impalib_type>>& rMatrix, vector<vector<int>>& rEdgeConnections, vector<vector<impalib_type>>& rFlippedMatrix){
