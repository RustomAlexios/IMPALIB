...
inline void EqualityConstraint::team_eq_constraint_to_oric_update(vector<vector<impalib_type>> &rExtrinsicOutputDepartment, vector<impalib_type> &rTeam2OricM, vector<impalib_type> &rewardTeam) const {
    ...
    for (int department_index = 0; department_index < rExtrinsicOutputDepartment.size(); department_index++) {
        transform(rExtrinsicOutputDepartment[department_index].begin(), rExtrinsicOutputDepartment[department_index].end(), intermediate_team_to_oric_m.begin(), intermediate_team_to_oric_m.begin(),
                  std::plus<impalib_type>());
    }
    transform(intermediate_team_to_oric_m.begin(), intermediate_team_to_oric_m.end(), rewardTeam.begin(), rTeam2OricM.begin(), std::plus<impalib_type>());
}
...
inline void EqualityConstraint::project_eq_constraint_to_oric_update(vector<vector<impalib_type>> &rProject2EqConstraintM, vector<vector<impalib_type>> &rEqConstraint2OricM,
                                                                     vector<vector<impalib_type>> &rewardProject) {
    for (int project_index = 0; project_index < rProject2EqConstraintM.size(); project_index++) {
        transform(rProject2EqConstraintM[project_index].begin(), rProject2EqConstraintM[project_index].end(), rewardProject[project_index].begin(), rEqConstraint2OricM[project_index].begin(),
                  std::plus<impalib_type>());
    }
}
...
inline void EqualityConstraint::edge_ec_to_degree_constraint_relaxed_graph_update(const vector<vector<int>> &rEdgeConnections, vector<vector<impalib_type>> &rEdgeDegreeConstraintCost,
    ...
    for (int edge_variable_index = 0; edge_variable_index < numEdgeVariables_; edge_variable_index++) {
        transform(flipped_degree_constraint_to_eq_constraint_m[edge_variable_index].begin(), flipped_degree_constraint_to_eq_constraint_m[edge_variable_index].end(),
                  rEdgeDegreeConstraintCost[edge_variable_index].begin(), rEdgeEc2DegreeConstraintM[edge_variable_index].begin(), plus<impalib_type>());
    }
}
...
inline vector<vector<impalib_type>> EqualityConstraint::edge_ec_to_subtour_constraints_update(const vector<vector<int>> &rDeltaSIndicesList, const vector<impalib_type> &rCostEdgeVaribale,
                                                                                              const vector<vector<impalib_type>> &rDegreeConstraint2EqConstraintM,
                                                                                              const vector<vector<impalib_type>> &rSubtourConstraints2EdgeEcM,
                                                                                              const vector<vector<int>> &rEdgeConnections) const {
    ...
    if (rDeltaSIndicesList.size() == 1) {
        ...
        for (size_t index_edge_variable = 0; index_edge_variable < numEdgeVariables_; ++index_edge_variable) {
            combined_degree_constraint_to_eq_constraint_m[index_edge_variable] = rDegreeConstraint2EqConstraintM[index_edge_variable][rEdgeConnections[index_edge_variable][0]] +
        ...
        for (size_t i = 0; i < rDeltaSIndicesList[0].size(); i++) {
            edge_ec_to_subtour_constraints_m[rDeltaSIndicesList[0][i]] = combined_degree_constraint_to_eq_constraint_m[rDeltaSIndicesList[0][i]] + rCostEdgeVaribale[rDeltaSIndicesList[0][i]];
        }
        ...
        for (size_t index_subtour_constraint = 0; index_subtour_constraint < rDeltaSIndicesList.size(); index_subtour_constraint++) {
            ...
            for (size_t i = 0; i < rDeltaSIndicesList[index_subtour_constraint].size(); i++) {
                edge_ec_to_subtour_constraints_m[rDeltaSIndicesList[index_subtour_constraint][i]] =
                    ...
                    combined_degree_constraint_to_eq_constraint_m[rDeltaSIndicesList[index_subtour_constraint][i]] + rCostEdgeVaribale[rDeltaSIndicesList[index_subtour_constraint][i]] -
                    rSubtourConstraints2EdgeEcM[index_subtour_constraint][rDeltaSIndicesList[index_subtour_constraint][i]];
            }
            edge_ec_to_subtour_constraints_list.push_back(edge_ec_to_subtour_constraints_m);
        }
    }
...
inline void EqualityConstraint::edge_ec_to_degree_constraint_augmented_graph_update(const vector<vector<impalib_type>> &rDegreeConstraint2EqConstraintM,
                                                                                    const vector<vector<impalib_type>> &rSubtourConstraints2EdgeEcM, const vector<vector<int>> &rEdgeConnections,
                                                                                    const vector<vector<impalib_type>> &rEdgeDegreeConstraintCost,
                                                                                    vector<vector<impalib_type>> &rEdgeEc2DegreeConstraintM) const {
    vector<impalib_type> combined_subtour_constraints_to_edge_ec_m(numEdgeVariables_, zero_value);
    for (const auto &row : rSubtourConstraints2EdgeEcM) {
        transform(combined_subtour_constraints_to_edge_ec_m.begin(), combined_subtour_constraints_to_edge_ec_m.end(), row.begin(), combined_subtour_constraints_to_edge_ec_m.begin(),
    ...
    for (size_t i = 0; i < rEdgeConnections.size(); i++) {
        ...
        rEdgeEc2DegreeConstraintM[i][rEdgeConnections[i][0]] =
            combined_subtour_constraints_to_edge_ec_m[i] + rDegreeConstraint2EqConstraintM[i][rEdgeConnections[i][1]] + rEdgeDegreeConstraintCost[i][rEdgeConnections[i][0]];

        // Update the message for the second node of the edge
        rEdgeEc2DegreeConstraintM[i][rEdgeConnections[i][1]] =
            combined_subtour_constraints_to_edge_ec_m[i] + rDegreeConstraint2EqConstraintM[i][rEdgeConnections[i][0]] + rEdgeDegreeConstraintCost[i][rEdgeConnections[i][1]];
    }
}
...
inline void EqualityConstraint::flip_matrix(const vector<vector<impalib_type>> &rMatrix, const vector<vector<int>> &rEdgeConnections, vector<vector<impalib_type>> &rFlippedMatrix) {
...
inline void EqualityConstraint::variable_ec_to_ksat_constraint_update(const vector<vector<impalib_type>> &rKsatConstraint2EqConstraintM_, vector<vector<impalib_type>> &rVariableEc2KsatConstraintM,
                                                                      vector<int> &rUsedVariables, const vector<impalib_type> &rIncomingMetricsCost,
                                                                      const vector<vector<int>> &rVariablesConnections) const {
    ...
    for_each(rUsedVariables.begin(), rUsedVariables.end(), [&](int n) { used_incoming_metrics_cost[n] = rIncomingMetricsCost[n]; });
