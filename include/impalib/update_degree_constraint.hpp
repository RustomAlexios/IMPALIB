...
inline void DegreeConstraint::degree_constraint_to_edge_ec_update(const vector<vector<impalib_type>> &rEdgeEc2DegreeConstraintM, const vector<vector<int>> &rEdgeConnections,
    ...
    for (int node_index = 0; node_index < numNodes_; node_index++) {
        ...
        for (int stage = 1; stage < connections_first.size(); stage++) {
            stage_forward_messages[connections_first[stage]] = min(stage_forward_messages[connections_first[stage - 1]], rEdgeEc2DegreeConstraintM[connections_first[stage - 1]][node_index]);
        }
        ...
        for (size_t stage = connections_first.size() - 1; stage >= 1; stage--) {
            stage_backward_messages[connections_first[stage - 1] + 1] = min(stage_backward_messages[connections_first[stage] + 1], rEdgeEc2DegreeConstraintM[connections_first[stage]][node_index]);
        }
        ...
        for (int index_edge_variable = 0; index_edge_variable < connections_first.size(); index_edge_variable++) {
            ...
            minimumValue = min(stage_forward_messages[connections_first[index_edge_variable]], stage_backward_messages[connections_first[index_edge_variable] + 1]);
        ...
        for (int stage = 1; stage < connections_second.size(); stage++) {
            stage_forward_messages[connections_second[stage]] = min(stage_forward_messages[connections_second[stage - 1]], rEdgeEc2DegreeConstraintM[connections_second[stage - 1]][node_index]);
        }
        ...
        for (size_t stage = connections_second.size() - 1; stage >= 1; stage--) {
            stage_backward_messages[connections_second[stage - 1] + 1] = min(stage_backward_messages[connections_second[stage] + 1], rEdgeEc2DegreeConstraintM[connections_second[stage]][node_index]);
        }

        for (int index_edge_variable = 0; index_edge_variable < connections_second.size(); index_edge_variable++) {
            impalib_type minimumValue = zero_value;
            minimumValue = min(stage_forward_messages[connections_second[index_edge_variable]], stage_backward_messages[connections_second[index_edge_variable] + 1]);
...
inline void DegreeConstraint::process_filtering(const int iter, vector<vector<impalib_type>> &rDegreeConstraint2EqConstraintDummyM, vector<vector<impalib_type>> &rDegreeConstraint2EqConstraintM) {
    for (int edge_variable_index = 0; edge_variable_index < numEdgeVariables_; edge_variable_index++) {
        if ((filteringFlag_) and (alpha_ != zero_value)) {
            // Calculate weighted values for current and old messages
            vector<impalib_type> intermediate_dummy(rDegreeConstraint2EqConstraintDummyM[edge_variable_index]), intermediate_old(degreeConstraint2EqConstraintOld[edge_variable_index]),
                intermediate_extrinsic;
            ...
            if (iter == 0) {
                copy(intermediate_dummy.begin(), intermediate_dummy.end(), rDegreeConstraint2EqConstraintM[edge_variable_index].begin());
            } else {
                ...
                copy(intermediate_extrinsic.begin(), intermediate_extrinsic.end(), rDegreeConstraint2EqConstraintM[edge_variable_index].begin());
            }
            copy(rDegreeConstraint2EqConstraintM[edge_variable_index].begin(), rDegreeConstraint2EqConstraintM[edge_variable_index].end(),
                 degreeConstraint2EqConstraintOld[edge_variable_index].begin());
        }

        else {
            copy(rDegreeConstraint2EqConstraintDummyM[edge_variable_index].begin(), rDegreeConstraint2EqConstraintDummyM[edge_variable_index].end(),
