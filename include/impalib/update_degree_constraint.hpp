...
class DegreeConstraint{
    ...
    public:
        void degree_constraint_to_edge_ec_update(vector<vector<impalib_type>>&, vector<vector<int>>&, vector<vector<impalib_type>>&);
        void process_filtering(int, vector<vector<impalib_type>> &, vector<vector<impalib_type>> &);
        
    DegreeConstraint(const int NUM_NODES, const int NUM_EDGE_VARIABLES, const bool FILTERING_FLAG, const impalib_type ALPHA);
};
...
void DegreeConstraint::degree_constraint_to_edge_ec_update(vector<vector<impalib_type>>& rEdgeEc2DegreeConstraintM, vector<vector<int>>& rEdgeConnections, vector<vector<impalib_type>> &rDegreeConstraint2EqConstraintDummyM){
        ...
        for (int node_index = 0; node_index <numNodes_; node_index++){
            ...
            for (int stage = connections_first.size()-1; stage >=1; stage--){
            ...
            for (int stage = connections_second.size()-1; stage >=1; stage--){
...
void DegreeConstraint:: process_filtering(int iter, vector<vector<impalib_type>> &rDegreeConstraint2EqConstraintDummyM, vector<vector<impalib_type>> &rDegreeConstraint2EqConstraintM){
        
        for (int edge_variable_index = 0; edge_variable_index < numEdgeVariables_; edge_variable_index++) {
            
            if ((filteringFlag_) and (alpha_ != zero_value)){
                
                vector<impalib_type> intermediate_dummy(rDegreeConstraint2EqConstraintDummyM[edge_variable_index]), intermediate_old(degreeConstraint2EqConstraintOld[edge_variable_index]), intermediate_extrinsic;
                ...
                copy(rDegreeConstraint2EqConstraintM[edge_variable_index].begin(), rDegreeConstraint2EqConstraintM[edge_variable_index].end(), degreeConstraint2EqConstraintOld[edge_variable_index].begin());
            }

            else{
                copy(rDegreeConstraint2EqConstraintDummyM[edge_variable_index].begin(), rDegreeConstraint2EqConstraintDummyM[edge_variable_index].end(), rDegreeConstraint2EqConstraintM[edge_variable_index].begin());
            }
        }
}
