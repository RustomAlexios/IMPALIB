...
inline SubtourEliminationConstraint::SubtourEliminationConstraint(const int NUM_NODES, const int NUM_EDGE_VARIABLES, const bool FILTERING_FLAG, const impalib_type ALPHA)
    : filteringFlag_(FILTERING_FLAG), alpha_(ALPHA), numNodes_(NUM_NODES), numEdgeVariables_(NUM_EDGE_VARIABLES), initial_forward_message_(value_inf), initial_backward_message_(value_inf){};
...
inline void SubtourEliminationConstraint::subtour_constraints_to_edge_ec_update(const vector<vector<impalib_type>> &rEdgeEc2SubtourConstraintsM, const vector<vector<int>> &rDeltaSIndicesList,
    ...
    for (size_t index_subtour_constraint = 0; index_subtour_constraint < rDeltaSIndicesList.size(); index_subtour_constraint++) {
        ...
        for (int index_edge_variable = 0; index_edge_variable < rDeltaSIndicesList[index_subtour_constraint].size(); index_edge_variable++) {
            ...
            minimumValue = min(stage_forward_messages[rDeltaSIndicesList[index_subtour_constraint][index_edge_variable]],
...
inline void SubtourEliminationConstraint::process_filtering(const int iter, vector<vector<impalib_type>> &rSubtourConstraints2EdgeEcDummyM, vector<vector<impalib_type>> &rSubtourConstraints2EdgeEcM,
    ...
    for (int index_subtour_constraint = 0; index_subtour_constraint < rDeltaSIndicesList.size(); index_subtour_constraint++) {
        if ((filteringFlag_) and (alpha_ != zero_value)) {
            vector<impalib_type> intermediate_dummy(rSubtourConstraints2EdgeEcDummyM[index_subtour_constraint]), intermediate_old(subtourConstraints2EdgeEcOld_[index_subtour_constraint]),
            ...
            copy(rSubtourConstraints2EdgeEcM[index_subtour_constraint].begin(), rSubtourConstraints2EdgeEcM[index_subtour_constraint].end(),
                 subtourConstraints2EdgeEcOld_[index_subtour_constraint].begin());
        }

        else {
            copy(rSubtourConstraints2EdgeEcDummyM[index_subtour_constraint].begin(), rSubtourConstraints2EdgeEcDummyM[index_subtour_constraint].end(),
