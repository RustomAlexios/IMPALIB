...
inline void InequalityConstraint::project_inequality_constraint_update(const vector<vector<impalib_type>> &rEqConstraint2ProjectM, vector<vector<impalib_type>> &rProject2EqConstraintM) const {
    ...
    for (int project_index = 0; project_index < rProject2EqConstraintM.size(); project_index++) {
        ...
        for (int stage = 0; stage < numTeams_; stage++) {
            ...
            stage_forward_messages_project_EC[stage + 1][1] =
                min(stage_forward_messages_project_EC[stage][1], stage_forward_messages_project_EC[stage][0] + rEqConstraint2ProjectM[project_index][stage]);
        }
        ...
        for (int stage = numTeams_ - 1; stage >= 0; stage--) {
            stage_backward_messages_project_EC[stage][0] =
                min(stage_backward_messages_project_EC[stage + 1][0], stage_backward_messages_project_EC[stage + 1][1] + rEqConstraint2ProjectM[project_index][stage]);
