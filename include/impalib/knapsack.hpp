...
inline void Knapsack::forward(const int department_index, vector<vector<impalib_type>> &rStageForwardMessages, const int max_state_department, vector<vector<int>> &rNonZeroWeightIndices,
                              const int *pNON_ZERO_WEIGHT_INDICES_SIZES_PY, const vector<vector<int>> &rTeamsWeightsPerDepartment, const vector<vector<impalib_type>> &rTeam2KnapsackM) const {
        ...
        for (int a = 0; a <= max_state_department; a++) {
            if (a - rTeamsWeightsPerDepartment[department_index][t] >= 0 && (!(t == 0 && a != rTeamsWeightsPerDepartment[department_index][t]))) {
                ...
                rStageForwardMessages[t + 1][a] =
                    min(initial_forward_messages[a], initial_forward_messages[a - rTeamsWeightsPerDepartment[department_index][t]] + rTeam2KnapsackM[department_index][t]);
            } else {
...
inline void Knapsack::backward(const int department_index, vector<vector<impalib_type>> &rStageBackwardMessages, const int max_state_department, vector<vector<int>> &rNonZeroWeightIndices,
                               const int *pNON_ZERO_WEIGHT_INDICES_SIZES_PY, const vector<vector<int>> &rTeamsWeightsPerDepartment, const vector<vector<impalib_type>> &rTeam2KnapsackM) const {
    vector<int>::iterator upper;
...
inline void Knapsack::extrinsic_output_department_lhs(const vector<vector<int>> &rTeamsWeightsPerDepartment, const vector<vector<impalib_type>> &rStageForwardMessages,
                                                      const vector<vector<impalib_type>> &rTeam2KnapsackM, const int department_index, const vector<vector<impalib_type>> &rStageBackwardMessages,
                                                      const int max_state_department, vector<vector<impalib_type>> &rExtrinsicOutputDepartment) {
    ...
    for (int team_index = 0; team_index < rTeamsWeightsPerDepartment[department_index].size(); team_index++) {
        ...
        if (rTeamsWeightsPerDepartment[department_index][team_index] != 0) {
            ...
            rExtrinsicOutputDepartment[department_index][team_index] =
                *min_element(metric_path_solid.begin(), metric_path_solid.end()) - *min_element(metric_path_dash.begin(), metric_path_dash.end()) - rTeam2KnapsackM[department_index][team_index];
        } else {
...
inline void Knapsack::team_to_knapsack_update(vector<vector<int>> &rNonZeroWeightIndices, vector<vector<impalib_type>> &rTeam2KnapsackM, const vector<impalib_type> &rRewardTeam,
                                              const vector<vector<impalib_type>> &rExtrinsicOutputDepartment, const vector<impalib_type> &mORIC2Team) const {
    for (int department_index = 0; department_index < rExtrinsicOutputDepartment.size(); department_index++) {
        ...
        for (auto t : remaining_departments) {
            ...
            it = set_intersection(rNonZeroWeightIndices[department_index].begin(), rNonZeroWeightIndices[department_index].end(), rNonZeroWeightIndices[t].begin(), rNonZeroWeightIndices[t].end(),
                                  intersection.begin());
...
inline void Knapsack::process_extrinsic_output_department(const int department_index, const int iter, vector<vector<impalib_type>> &rExtrinsicOutputDepartmentDummy,
