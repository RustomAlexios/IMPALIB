...
inline void Knapsack::forward(const int department, vector<vector<impalib_type>> &forward,
                       const int capacity, vector<vector<int>> &nonzeroWeights,
                       const int *pNON_ZERO_WEIGHT_INDICES_SIZES_PY, const vector<vector<int>> &weights,
                       const vector<vector<impalib_type>> &team2KnapsackM) const
{
    ...
    for (int l = 0; l < pNON_ZERO_WEIGHT_INDICES_SIZES_PY[department]; l++)
    {
        ...
        {
            if (a - weights[department][t] >= 0
                && (!(t == 0 && a != weights[department][t])))
            {
                // Update forward messages
                forward[t + 1][a] =
                    min(forward0[a],
                        forward0[a - weights[department][t]]
                            + team2KnapsackM[department][t]);
            }
            else
            {
        ...
        if (nonzeroWeights[department].size() != numTeams_)
        {
                               ...
                               nonzeroWeights[department].end(), t + 1))
            {
                ...
                {
                    upper             = upper_bound(nonzeroWeights[department].begin(),
                                                    nonzeroWeights[department].end(), t);
...
inline void Knapsack::backward(const int department, vector<vector<impalib_type>> &backward,
                        const int capacity, vector<vector<int>> &nonzeroWeights,
                        const int *pNON_ZERO_WEIGHT_INDICES_SIZES_PY, const vector<vector<int>> &weights,
                        const vector<vector<impalib_type>> &team2KnapsackM) const
{
    ...
    for (int l = pNON_ZERO_WEIGHT_INDICES_SIZES_PY[department] - 1; l >= 0; l--)
    {
        ...
        for (int a = 0; a <= capacity; a++)
        {
            if ((t > 0 && a + weights[department][t] <= capacity)
                || (a == 0 && t == 0))
            {
                // Update backward messages
                backward[t][a] =
                    min(backward0[a],
                        backward0[a + weights[department][t]]
                            + team2KnapsackM[department][t]);
            }
            else
            {
...
inline void Knapsack::extrinsic_output_department_lhs(const vector<vector<int>>          &weights,
                                               const vector<vector<impalib_type>> &forward,
                                               const vector<vector<impalib_type>> &team2KnapsackM, const int department,
                                               const vector<vector<impalib_type>> &backward,
                                               const int                           capacity,
        ...
        if (weights[department][team] != 0)
        {
            ...
            if (team == 0)
            {
                metric_path_solid.push_back(
                    forward[team][team]
                    + backward[team + 1][weights[department][team]]
                    + team2KnapsackM[department][team]);
                metric_path_dash.push_back(forward[team][team]
                                           + backward[team + 1][0]);
            }
            else
            {
                for (int i = 0; i <= capacity; i++)
                {
                    if (i + weights[department][team] <= capacity)
                    {
                        metric_path_solid.push_back(
                            forward[team][i]
                            + backward[team + 1]
                                                    [i + weights[department][team]]
                            + team2KnapsackM[department][team]);
                    }
                    metric_path_dash.push_back(forward[team][i]
                                               + backward[team + 1][i]);
                }
            }
...
inline void Knapsack::team_to_knapsack_update(vector<vector<int>>          &nonzeroWeights,
                                       vector<vector<impalib_type>> &team2KnapsackM, const vector<impalib_type> &rewards,
                                       const vector<vector<impalib_type>> &extrisicOut,
                                       const vector<impalib_type>         &oric2TeamM) const
{
    for (int department = 0; department < extrisicOut.size(); department++)
    {
        ...
        for (auto t : remaining_departments)
        {
            ...
            it = set_intersection(nonzeroWeights[department].begin(),
                                  ...
                                  nonzeroWeights[t].end(), intersection.begin());
            ...
            for (auto l : intersection)
            {
                ...
                team2KnapsackM[department][l] =
                    rewards[l] + extrisicOut[t][l] + oric2TeamM[l];
...
inline void Knapsack::process_extrinsic_output_department(const int department, const int iter,
                                                   vector<vector<impalib_type>> &extrinsicOutPre,
                                                   vector<vector<impalib_type>> &extrinsicOut)
{
    ...
    {
        copy(extrinsicOutPre[department].begin(),
             extrinsicOutPre[department].end(),
             extrinsicOut[department].begin());
    }
}